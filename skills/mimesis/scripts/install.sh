#!/bin/sh
# Install mimesis skills for any Agent Skills-compatible tool
...
# Symlinked skills resolve ../../reference and ../../linter back into the
# repo, so nothing else needs installing.
#
# Flags:
#   --project    install into ./.agents/skills/ of the current directory
#   --copy       copy instead of symlink (for filesystems/tools without
#                symlinks); also copies reference/, linter and codex.md into
#                the target's parent so the skills' relative paths resolve
...
for dir in "$TARGET" $EXTRA_TARGETS; do
  ...
  # In copy mode, the skills' ../../ paths point at the dir's parent;
  # mirror the repo layout there so reference reads and linter runs work.
  if [ "$UNINSTALL" -eq 0 ] && [ "$MODE" = copy ]; then
    parent=$(dirname "$dir")
    rm -rf "$parent/reference" "$parent/linter" "$parent/codex.md"
    cp -R "$REPO_DIR/reference" "$parent/reference"
    cp "$REPO_DIR/linter" "$parent/linter"
    cp "$REPO_DIR/codex.md" "$parent/codex.md"
    echo "installed reference/, linter, codex.md -> $parent"
  fi
done
